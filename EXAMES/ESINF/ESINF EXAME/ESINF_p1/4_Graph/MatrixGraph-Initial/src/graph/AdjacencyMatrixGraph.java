...
public class AdjacencyMatrixGraph<V, E> implements BasicGraph<V, E>, Cloneable {
    ...
    public Iterable<V> directConnections(V vertex) {
        ...
        Set<V> directVertex = new HashSet<>();
        for (E edge : this.edges()) {
            V[] endVertices = this.endVertices(edge);
            if (endVertices[0].equals(vertex)) {
                directVertex.add(endVertices[1]);
            } else if (endVertices[1].equals(vertex)) {
                directVertex.add(endVertices[0]);
            }
        }
        return directVertex;
    }
    ...
    public Iterable<E> outgoingEdges(V vertex) {
        int posicao = toIndex(vertex);
        if (posicao == -1) {
            return null;
        }
        Set<E> edges = new HashSet<>();
        for (int j = 0; j < this.edgeMatrix[0].length; j++) {
            if (edgeMatrix[posicao][j] != null) {
                edges.add(edgeMatrix[posicao][j]);
            }
        }
        return edges;
    }

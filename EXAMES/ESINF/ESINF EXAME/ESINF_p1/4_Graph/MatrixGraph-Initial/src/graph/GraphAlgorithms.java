...
public class GraphAlgorithms {
    ...
    public static <V, E> LinkedList<V> BFS(AdjacencyMatrixGraph<V, E> graph, V vertex) {
        int vertIndex = graph.toIndex(vertex);
        if (vertIndex == -1) {
            return null;
        }
        ...
        resultQueue.add(vertex);
        auxQueue.add(vertex);
        knownVertices[vertIndex] = true;

        while (!auxQueue.isEmpty()) {
            V vertOrig = auxQueue.peek();
            for (V vertAdj : graph.directConnections(vertOrig)) {
                int adgIndex = graph.toIndex(vertAdj);
                if (knownVertices[adgIndex] == false) {
                    resultQueue.add(vertAdj);
                    auxQueue.add(vertAdj);
                    knownVertices[adgIndex] = true;
                }
            }
    ...
    public static <V, E> AdjacencyMatrixGraph<V, E> transitiveClosure(AdjacencyMatrixGraph<V, E> graph, E dummyEdge) {
        ...
        AdjacencyMatrixGraph<V, E> matrix = (AdjacencyMatrixGraph) graph.clone();
        for (int i = 0; i < graph.numVertices(); i++) {
            for (int j = 0; j < graph.numVertices(); j++) {
                E edge = graph.edgeMatrix[i][j];
                if (i != j && edge != null) {
                    for (int k = 0; k < graph.numVertices(); k++) {
                        if (k != i && k != j && graph.edgeMatrix[i][k] != null) {
                            graph.edgeMatrix[j][k] = dummyEdge;
                        }
                    }
